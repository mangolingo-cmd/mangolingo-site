...
- ✅ **OlympusStaff migration + covers fix** (Jun 2026): manga-spark.net is DEAD (connection reset, even curl_cffi). Chapter CDN (s3sparkio/leksparkio) still works so old chapters remain readable. New Arabic source: olympustaff.com (~1,137 series, images inline in HTML, curl_cffi works). Added `backend/scrape_olympustaff.py` (catalog crawl, series meta AR genres→EN map, chapter listing paginated ?page=N, smart_import with match-by-normalized title/slug + budgets, refresh_latest). New endpoint `POST /api/admin/import-olympustaff?max_new_titles=10&max_chapters=400` (background, kind=admin_olympus_import, re-runnable/idempotent — each run continues). Admin.jsx: "استيراد من OlympusStaff" button + progress card. Extended fix-covers job: now also targets dead manga-spark.net covers and searches MangaDex BY NAME for titles without mangadex_id (fixed 164/197 in preview; olympus import fixes rest via olympus covers on match). 6h scheduler + "تحديث الآن" now: preflight manga-spark (skip if dead) + olympus refresh_latest (titles with olympus_slug). Proxy allowlist += olympustaff.com. azorafly.com rejected (JS-loaded images, small catalog). PRODUCTION steps after redeploy: click "إصلاح الأغلفة" once, then "استيراد من OlympusStaff" repeatedly (each run ≤10 new titles/≤400 chapters).
